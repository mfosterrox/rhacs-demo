...
Include `GRPC_ENFORCE_ALPN_ENABLED=false` so that roxctl gRPC calls (e.g. image scan) succeed when the TLS handshake does not advertise ALPN (avoids "missing selected ALPN property" errors with OpenShift routes).
...
[start=3]
. Install Podman on the bastion host if it is not already installed.

To proceed, use the installation instructions that match your operating system (for example, RHEL, Fedora, CentOS, Ubuntu, or Debian).

Install Podman using the appropriate command for your distribution:
...
[source,sh,subs="attributes",role=execute]
----
sed -i "s|quay.io/mfoster/frontend:latest|$QUAY_URL/$QUAY_USER/frontend:0.1|g" $APP_HOME/frontend.yml
----
