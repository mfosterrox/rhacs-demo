...
control-plane-cluster-8sgd9-1   Ready    control-plane,master,worker   176m   v1.33.6
worker-cluster-8sgd9-1          Ready    worker                        120m   v1.33.6
worker-cluster-8sgd9-2          Ready    worker                        120m   v1.33.6
----
...
[source,sh,subs="attributes",role=execute]
----
curl -k -u "admin:{acs_portal_password}" "https://{acs_route}:443/v1/apitokens/generate" \
