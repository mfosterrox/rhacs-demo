...
----
oc delete -f $APP_HOME/frontend.yml
----

*Sample output*
[source,texinfo,subs="attributes"]
----
[lab-user@bastion ~]$ oc delete -f $APP_HOME/frontend.yml
...
[start=5]
. Click *Next*
. Next, deselect *Build & Deploy* stage so that only the Deploy stage is selected:
...
[source,sh,subs="attributes",role=execute]
----
roxctl -e $ROX_CENTRAL_ADDRESS:443 deployment check --file $APP_HOME/frontend.yml --insecure-skip-tls-verify
----
...
====
If you need to bypass the policy (for example, during an emergency), add the break-glass annotation to the Deployment in the YAML file before applying, then apply the file.
====

[source,sh,subs="attributes",role=execute]
----
sed -i '/^kind: Deployment$/,/^spec:/{/^metadata:$/a\
  annotations:\
    admission.stackrox.io/break-glass: "ticket-1234"
}' $APP_HOME/frontend.yml
oc apply -f $APP_HOME/frontend.yml
----
...
[source,bash,subs="+macros,+attributes"]
----
[lab-user@bastion ~]$ sed -i '/^kind: Deployment$/,/^spec:/{/^metadata:$/a\
  annotations:\
    admission.stackrox.io/break-glass: "ticket-1234"
}' $APP_HOME/frontend.yml
oc apply -f $APP_HOME/frontend.yml
deployment.apps/frontend created
route.route.openshift.io/frontend-patient-route unchanged
