...
----
cd ~/
export TUTORIAL_HOME="$(pwd)/demo-appapplications"
...
cat $TUTORIAL_HOME/k8s-deployment-manifests/medical-application/backend/deployment-backend-atlas.yaml
----
...
[source,sh,subs="attributes",role=execute]
----
roxctl netpol connectivity map $TUTORIAL_HOME/k8s-deployment-manifests/medical-application/backend/
----
...
[source,sh,subs="attributes",role=execute]
----
touch $TUTORIAL_HOME/k8s-deployment-manifests/medical-application/backend/network-policies.yml
roxctl netpol generate $TUTORIAL_HOME/k8s-deployment-manifests/medical-application/backend/ >> $TUTORIAL_HOME/k8s-deployment-manifests/medical-application/backend/network-policies.yml
----
...
[source,sh,subs="attributes",role=execute]
----
oc apply -f $TUTORIAL_HOME/k8s-deployment-manifests/medical-application/backend/network-policies.yml
----
...
[source,bash,subs="+macros,+attributes"]
----
INFO:   in file: /home/lab-user/demo-apps/k8s-deployment-manifests/medical-application/backend/everything.yml, skipping object with type: Secret
...
...
Use the 'roxctl netpol generate' command on the rest of the Kubernetes manifests in the $TUTORIAL_HOME/k8s-deployment-manifests/ folder. Apply these changes and view the updates in the Network Graph UI. 
