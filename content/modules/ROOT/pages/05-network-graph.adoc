...
image::06-network-graph-overview.png[Network Graph showing deployment-level traffic flows across Payments and other namespaces,link=self,window=blank,width=700,title="Network Graph with multiple namespaces selected"]
...
image::06-network-deployment-flows.png[Deployment detail panel for visa-processor showing one anomalous flow and three baseline flows,link=self,window=blank,width=700,title="Deployment network flows showing anomalous connection"]
...
image::06-network-policy-generator.png[Network Policy Simulator showing generated Kubernetes Network Policy YAML for the Payments namespace,link=self,window=blank,width=700,title="Network Policy Simulator output"]
