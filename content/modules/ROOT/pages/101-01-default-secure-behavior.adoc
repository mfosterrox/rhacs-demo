...
== Concepts: Non-Root and Privileged Ports

OpenShift does not assume your container image was built with security in mind. It assigns a *random non-root UID* to every pod (via the restricted Security Context Constraint) and does not allow binding to *privileged ports* (e.g., 80, 443) without elevated capabilities. So an image that "works on my laptop" as root on port 80 will often fail on OpenShift: the process cannot bind to port 80 and may not be able to write to root-owned log directories. That failure is intentional—it surfaces insecure assumptions early. The fix is to use or build images that run as a non-root user and listen on a high port (e.g., 8080). Red Hat UBI (Universal Base Image) variants like `ubi9/httpd-24` are designed for this: they run as non-root and serve on 8080 by default, so they work with OpenShift's guardrails without any SCC exceptions.

== How to Try It

You will create a project, deploy the public Docker Hub `httpd` image (which assumes root and port 80), observe the failure and inspect logs and events. Then you will replace the image with a Red Hat UBI httpd image (non-root, port 8080), expose a Route, and verify the service works—all without granting anyuid or changing SCCs.

=== Use case 1: Create project and deploy external image
Create a dedicated project and deploy the public Docker Hub `httpd` image. OpenShift accepts the deployment, but the container will fail at runtime because it expects root and port 80 while the platform enforces a random non-root UID and blocks privileged port binds.

*Procedure*

. Create the project and deployment:

[source,sh]
----
oc new-project 101-01-httpd-demo
oc create deployment web -n 101-01-httpd-demo --image=httpd
----
...
[source,sh]
----
oc get pods -l app=web -n 101-01-httpd-demo
oc get pods -l app=web -n 101-01-httpd-demo -o wide
----

*Expected:* Pod in `CrashLoopBackOff` or `Error` with a non-zero restart count.

=== Use case 2: Observe failure — logs and events
Inspect why the container failed using logs and cluster events. This is your first line of evidence for "why didn't this work?" and for compliance or incident review.

*Procedure*

. Fetch recent logs from the web pods:

[source,sh]
----
oc logs -l app=web -n 101-01-httpd-demo --tail=50 --all-containers=true
----
...
. Check recent events and pod description for SCC, image pull, or runtime messages:

[source,sh]
----
oc get events -n 101-01-httpd-demo --sort-by='.lastTimestamp' | tail -20
oc describe pod -l app=web -n 101-01-httpd-demo | tail -30
----

TIP: Port 80 requires elevated privileges; default log directories are root-owned. OpenShift injects a random non-root UID, so the process cannot bind to 80 or write to those paths. The correct fix is to use a high port (e.g., 8080) and an image that supports non-root—not to grant anyuid.

=== Use case 3: Confirm runtime user and port assumption
Inspect the pod spec to see that OpenShift has applied a non-root security context. The image expects root and port 80, but the platform overrides the user; the mismatch causes the failure.

*Procedure*

. Print the pod's security context and container port:

[source,sh]
----
oc get pod -l app=web -n 101-01-httpd-demo -o jsonpath='{.items[0].spec.securityContext}' ; echo
oc get pod -l app=web -n 101-01-httpd-demo -o jsonpath='Container port: {.items[0].spec.containers[0].ports[0].containerPort}{"\n"}' ; echo
----

*Expected:* A numeric `runAsUser` (or similar) in the security context (e.g., 1000690000) and port 80 in the container spec. The platform assigns a random non-root UID while the image was built assuming root and binding to port 80, so the process fails.

=== Use case 4: Replace image with trusted UBI httpd
Replace the image in-place with a Red Hat UBI image that is designed for non-root and listens on 8080. No privilege escalation or SCC changes are required.
...
. Set the deployment image to the UBI httpd image and wait for rollout:

[source,sh]
----
oc set image deployment/web -n 101-01-httpd-demo *=registry.access.redhat.com/ubi9/httpd-24
oc rollout status deployment/web -n 101-01-httpd-demo --timeout=120s
----
...
----
oc get pods -l app=web -n 101-01-httpd-demo
oc logs -l app=web -n 101-01-httpd-demo --tail=15
----

*Expected:* Pod Running; logs show httpd listening on 8080. No permission errors.

IMPORTANT: Prefer fixing the image (or using a curated one like UBI) over granting anyuid. Short-term anyuid may "fix" the failure, but it normalizes root and expands blast radius; fix the workload instead.

=== Use case 5: Verify non-root UID and expose the service
...
. Check the runtime user inside the container:

[source,sh]
----
oc exec deploy/web -n 101-01-httpd-demo -- id
oc exec deploy/web -n 101-01-httpd-demo -- id -u
----

*Expected:* A numeric UID such as `1000690000` (not `0`).

. Expose the deployment as a Route and note the hostname:

[source,sh]
----
oc expose deployment web -n 101-01-httpd-demo --port=8080
oc get route web -n 101-01-httpd-demo -o jsonpath='{.spec.host}{"\n"}'
----

=== Use case 6: Test the route from the command line
Use `curl` to hit the Route and confirm the UBI httpd response. If you are on a host that can resolve the cluster's ingress DNS, the commands below work directly; otherwise use the cluster's default ingress or port-forward from a machine that can reach the route host.

*Procedure*

. Set the route hostname and test with curl:

[source,sh]
----
ROUTE=$(oc get route web -n 101-01-httpd-demo -o jsonpath='{.spec.host}')
curl -s -o /dev/null -w "%{http_code}" "http://$ROUTE/" && echo " OK"
curl -s "http://$ROUTE/" | head -5
----

*Expected:* HTTP 200 and HTML from the UBI httpd default page.

NOTE: If your host cannot resolve the cluster's internal DNS, use `oc get route web -n 101-01-httpd-demo -o jsonpath='{.spec.host}'` and open that URL in a browser (or run curl from a machine that can resolve it), or use `oc port-forward` to test locally.
...
You deployed a root-and-port-80 image, saw it fail under OpenShift's guardrails, then switched to a UBI image that runs non-root on 8080 and succeeded without any SCC changes. In your own environment, consider:

* How would you standardize on non-root, high-port images in your pipelines (e.g., Dockerfile lint, base image policy)?
* Would you use RHACS or image scanning to flag images that run as root or require privileged ports before they reach production?
* How would you handle a legacy or vendor image that cannot be rebuilt—document an exception with risk acceptance, or still push for a high-port variant?
...
You deployed the public httpd image and saw it fail under OpenShift's default guardrails (random non-root UID, no privileged port 80). You inspected logs and the pod spec to confirm the cause, then replaced the image with Red Hat UBI httpd (non-root, port 8080), exposed a Route, and verified the service. No SCC exceptions were required—the fix was choosing an image built for secure defaults. OpenShift's "secure-by-default" approach turns invisible risk into clear feedback; prefer fixing images over granting anyuid.
