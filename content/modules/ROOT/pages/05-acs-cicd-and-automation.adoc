...
* Understand and implement CI/CD integration with Red Hat Advanced Cluster Security (RHACS)
* Learn how to scan container images and deployments as part of a CI/CD pipeline
* Automate policy enforcement and compliance checks in a DevSecOps workflow
* Use RHACS APIs and CLI tools (roxctl) for automation within CI pipelines
* Implement policy-as-code for security automation with Advanced Cluster Management (ACM) and OpenShift GitOps
...
Click on *Fleet Management* in the left side menu.
...
[start=7]
. Click "*Next*"
. In the Template tab, select *Git Repository* and enter the URL of the GitHub repository containing the custom policies
+
[source,sh,subs="attributes",role=execute]
----
https://github.com/mfosterrox/skupper-security-demo.git
----
+
IMPORTANT: Make sure to click "Create new option"
+
. Select "Main":
...
. To get to your cluster's local OpenShift dashboard, click the top dropdown and select *Administrator*:
+
...
[NOTE]
====
This error is a known OpenShift Pipelines (Tekton) controller issue (validation webhook vs. finalizer cleanup), not a problem with your pipeline or tasks. If `oc delete` hangs or you see _Failed to update finalizers for "rox-pipeline-…": the object has been modified; please apply your changes to the latest version and try again_, a stuck PipelineRun is holding finalizers. Remove them and retry:
+
. Find the stuck PipelineRun. Check the pipeline namespace first, then all namespaces if needed:
+
[source,sh,subs="attributes",role=execute]
----
oc get pipelinerun -n pipeline-demo
# If no resources found, the run may be in another namespace (e.g. where you started the pipeline):
oc get pipelinerun -A
----
+
. If you see a PipelineRun listed, patch it using that run's *name* and *namespace* (not necessarily `pipeline-demo`). If no PipelineRuns are found, skip to step 3.
+
[source,sh,subs="attributes",role=execute]
----
kubectl patch pipelinerun <pipelinerun-name> -n <namespace> --type=json -p '[{"op": "remove", "path": "/metadata/finalizers"}]'
----
+
. Run the delete and apply again.
====
+
[.console-output]
[source,bash,subs="+macros,+attributes"]
