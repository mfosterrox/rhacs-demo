...
* VM SSH access: `virtctl ssh cloud-user@vmi/rhel-webserver -n default` (KubeVirt v1.6+ syntax; uses bastion SSH keys). Wait 5–10 minutes after the VM is Running for cloud-init to complete. If SSH fails, VMs may need to be recreated so cloud-init picks up the bastion key.
