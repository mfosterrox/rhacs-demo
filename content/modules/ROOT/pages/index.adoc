...
* VM access: Console `virtctl console rhel-webserver -n default` (cloud-user + Enter). SSH `virtctl ssh -i ~/.ssh/id_ed25519 cloud-user@vmi/rhel-webserver -n default` (requires bastion key in cloud-init; virtctl ssh does not support password). Wait 5–10 minutes after the VM is Running for cloud-init to complete.
