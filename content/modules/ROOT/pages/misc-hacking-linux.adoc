...
IMPORTANT: Use the OpenShift Route URL in your browser. Routes created with `oc expose svc` typically have no TLS (check with `oc get route` — TERMINATION will show `None`). Use *http://* (not https) for the hostname, e.g. `http://ctf-web-to-system-service-ctf-web-to-system.<apps-domain>/`. If your cluster uses TLS for routes, use https instead.
