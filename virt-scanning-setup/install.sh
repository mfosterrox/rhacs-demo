...
display_summary() {
    ...
    if [ "${SKIP_SUBSCRIPTION:-false}" != "true" ] && { [ -n "${RHEL_USERNAME:-}" ] || [ -n "${RHEL_ORG:-}" ]; }; then
        ...
        echo ""
        echo "  3. (Optional) SSH into VM:"
        echo "     $ virtctl ssh cloud-user@vmi/rhel-webserver -n default"
        ...
        echo "     $ oc get vmi -n default"
        echo ""
        echo "  2. SSH into VMs (uses bastion key):"
        echo "     $ virtctl ssh cloud-user@vmi/rhel-webserver -n default"
        echo ""
...
install_virtctl() {
    ...
    if [ -f "$install_path" ] && [ -x "$install_path" ]; then
        ...
        echo "  • virtctl ssh cloud-user@vmi/<vm-name> -n <ns>  # SSH into VM (v1.6+ syntax)"
